...
export default function DropdownItems({ label, icon, index, link }: Iprops) {
    ...
    return (
        <div onClick={execute} className={`flex items-center gap-2 text-[.8rem] text-gray-700 hover:text-gray-950 py-1 hover:bg-slate-200 cursor-pointer px-3 pl-7 rounded-md ${itemState === index && `bg-slate-200 text-gray-950`}`}>
            {icon}
